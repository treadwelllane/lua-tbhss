...
cmd_process_snli:option("--input", "Stanford NLI formatted input file", nil, nil, "+", 1)
cmd_process_snli:option("--output", "File to write triplets to", nil, nil, 1, 1)
cmd_process_snli:option("--quality", "validated majority size, number of majority pairs", nil, tonumber, 2, 1)
cmd_process_snli:flag("--include-entailment-as-premise", "include (pos, anchor, neg) triplets as well as (anchor, pos, neg) triplets")
...
cmd_load_train_triplets:option("--merge", "whether or not to merge repeated tokens", nil, fun.bind(op.eq, "true"), 1, 1):choices({ "true", "false" })
...
cmd_create_encoder:option("--persist-file", "the file to persist the trained model to", nil, nil, 1, 1)
cmd_create_encoder:option("--persist-state", "persist model weights (for additional training) or not (for inference only)", nil, fun.bind(op.eq, "true"), 1, 1):choices({ "true", "false" })
