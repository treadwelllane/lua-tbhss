...
int tbhss_blas_vector (lua_State *L)
{
  ...
  if (lua_type(L, 1) == LUA_TNUMBER) {
    ...
    tbhss_blas_vector_t *vec = (tbhss_blas_vector_t *)
      lua_newuserdata(L, sizeof(tbhss_blas_vector_t) + sizeof(double) * size); // tbl vec
    vec->size = size;
...
int tbhss_blas_average (lua_State *L)
{
  ...
  tbhss_blas_vector_t *v0 = (tbhss_blas_vector_t *) luaL_checkudata(L, -1, TBHSS_BLAS_VECTOR_MT);
  size_t dims = v0->size;
  lua_pop(L, 1); // tbl
  tbhss_blas_vector_t *out = (tbhss_blas_vector_t *)
    lua_newuserdata(L, sizeof(tbhss_blas_vector_t) + sizeof(double) * dims); // tbl vec
  out->size = dims;
  cblas_dcopy(v0->size, v0->data, 1, out->data, 1);
  for (size_t i = 2; i <= nvecs; i ++) {
    lua_pushinteger(L, i); // tbl vec i
    lua_gettable(L, -3); // tbl vec vec
    tbhss_blas_vector_t *v1 = (tbhss_blas_vector_t *) luaL_checkudata(L, -1, TBHSS_BLAS_VECTOR_MT);
...
int tbhss_blas_vector_dot (lua_State *L)
{
  lua_settop(L, 2);
  tbhss_blas_vector_t *v0 = (tbhss_blas_vector_t *) luaL_checkudata(L, 1, TBHSS_BLAS_VECTOR_MT);
  tbhss_blas_vector_t *v1 = (tbhss_blas_vector_t *) luaL_checkudata(L, 2, TBHSS_BLAS_VECTOR_MT);
...
int tbhss_blas_from_raw (lua_State *L)
{
  ...
  tbhss_blas_vector_t *out = (tbhss_blas_vector_t *)
    lua_newuserdata(L, sizeof(tbhss_blas_vector_t) + sizeof(double) * doubles); // tbl vec
  out->size = doubles;
